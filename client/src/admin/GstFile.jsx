...
import Layout from "../components/dashboard/Layout"; // Adjust path if necessary based on folder structure
...
// ==========================================
// INVOICE TEMPLATE (For Printing Individual Bills)
// ==========================================
            ...
            <div className="mb-4 border border-gray-300 p-3 rounded mt-4">
                <div className="flex justify-between">
                    ...
                    <div className="w-1/2 text-right">
                        ...
                        <p className="text-gray-700 text-xs">Status: <b>{bill.orderStatus?.delivered ? "DELIVERED" : "ORDERED"}</b></p>
                    </div>
                </div>
            </div>
...
const GstFile = () => {
    ...
    // --- FETCH ALL BILLS (No Filter) ---
    const fetchAllBills = async () => {
        try {
            setLoading(true);
            // Calling API without ?type=delivered fetches everything
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(workbook, `GST_Register_${new Date().toISOString().split('T')[0]}.xlsx`);
            ...
            <div className="bg-white shadow-md rounded-lg p-6 min-h-screen">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800">GST File / All Bills</h2>
                            <p className="text-sm text-gray-500">Complete register of all invoices</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100 items-end">
                    ...
                    <div className="relative w-full sm:max-w-[150px]">
                        ...
                        <input
                            type="date"
                            className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none text-gray-600"
                            value={fromDate}
                ...
                <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200 shadow-sm bg-white p-2">
                    <table className="min-w-full text-xs text-left text-gray-700">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan="12" className="p-6 text-center text-gray-500 animate-pulse">Loading data...</td></tr>
                                ...
                                displayedRows.map((row) => (
                                    <tr key={row._id} className="bg-white border-b hover:bg-blue-50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-right font-bold text-gray-800">{safeAmount(row.total)}</td>
